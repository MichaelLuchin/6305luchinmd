"""
Модуль для обработки данных экспонатов Метрополитен-музея.
Выполняет чанковую загрузку, очистку и визуализацию статистики по культурам.
"""
...
def read_chunks(
    ...
    Yields:
        pd.DataFrame: Чанк данных с выбранными столбцами.
    """
...
def clean_and_calc_chunks(
    ...
    Args:
        chunks: Генератор сырых чанков данных.

    Yields:
        pd.DataFrame: Очищенные данные с вычисленным возрастом.
    """
    for chunk in chunks:
        # Извлекаем год поступления (первые 4 цифры)
        chunk['AccessionYearClean'] = (
            chunk['AccessionYear'].astype(str).str.extract(r'(\d{4})')[0].astype(float)
        )
        # Приводим даты к числам
        ...
        chunk = chunk.dropna(subset=['Culture',
                                     'AccessionYearClean', 'Object Begin Date'])

        # Расчет возраста на момент поступления
        chunk['Age'] = chunk['AccessionYearClean'] - chunk['Object Begin Date']

        # Фильтруем аномалии (возраст не может быть отрицательным)
...
def local_aggregate_chunks(
    ...
    Args:
        chunks: Генератор обработанных чанков.

    Yields:
        tuple[pd.DataFrame, pd.DataFrame]: Локальная статистика и временной ряд.
    """
    for chunk in chunks:
        chunk['Age_sq'] = chunk['Age'] ** 2

        # Статистика для столбцовой диаграммы
        ...
        # Данные для временного ряда (группировка по культуре и году)
...
def collect_global_stats(
    ...
    Args:
        local_agg_gen: Генератор локальных статистик по чанкам.

    Returns:
        tuple[pd.DataFrame, pd.DataFrame]: Глобальная статистика и временной ряд.
    """
    df_stats = None
    df_timeline = None
...
def main() -> None:
    """
    Основная функция: запуск пайплайна обработки и визуализация.
    """
    file_path = "MetObjects.csv"

    raw_chunks = read_chunks(file_path, chunk_size=10000)
    ...
    # Поиск культуры с самой длительной историей
    ...
    timeline['rolling_mean'] = timeline['mean_age'].rolling(
        window=5,
        min_periods=1,
    ).mean()
